...
class InventoryController extends Controller
{
    ...
    public function getManageStockPageLoad() {
        ...
        return view('admin.manage-stock', compact('AllCount', 'InStockCount', 'OutOfStockCount', 'datalist'));		
	}
	...
	public function getManageStockTableData(Request $request){
		...
		if($request->ajax()){
			...
			if($search != ''){
				...
				$datalist =  Product::where(function ($query) use ($status){
						$query->whereRaw("stock_status_id = '".$status."' OR '".$status."' = '2'");
					})
					->orderBy('id','desc')
					->paginate(20);
			...
			return view('admin.partials.manage_stock_table', compact('datalist'))->render();
		}
	}
