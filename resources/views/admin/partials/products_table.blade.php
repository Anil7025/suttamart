	...
	<table class="table table-borderless table-theme" style="width:100%;">
		...
		<tbody>
			...
			<tr>
				...
				<td class="text-left"><a href="{{ route('admin.product', [$row->id]) }}" title="{{ __('Edit') }}">{{ $row->title }}</a></td>
				...
				@if ($row->image != '')
				<td class="text-center"><div class="table_col_image"><img src="{{ asset('uploads/products') }}/{{ $row->image }}" /></div></td>
				@else
				...
				<td class="text-center">
					<div class="btn-group action-group">
						...
						<div class="dropdown-menu dropdown-menu-right">
							<a class="dropdown-item" href="{{ route('admin.product', [$row->id]) }}">{{ __('Edit') }}</a>
