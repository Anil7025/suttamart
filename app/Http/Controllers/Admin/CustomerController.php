...
class CustomerController extends Controller
{
    ...
    public function getCustomersPageLoad(){
		...
		$datalist = DB::table('users')
			->select('users.id', 'users.name', 'users.email', 'users.mobile', 'users.address', 'users.image', 'users.status', )
			->where('status', '=', 'active')
			->orderBy('users.name','asc')
			->paginate(15);
			
        return view('admin.customers', compact('statuslist', 'datalist'));
    }
	...
	public function getCustomersTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view('admin.partials.customers_table', compact('datalist'))->render();
		}
	}
