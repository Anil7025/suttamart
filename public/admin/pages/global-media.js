...
function onGlobalMediaModalDataLoad() {
	$.ajax({
		url:base_url + "/admin/getGlobalMediaData",
...
function onGlobalMediaPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/admin/getGlobalMediaData?page="+page,
		success:function(data){
			$('#global_datalist').html(data);
		}
	});
...
function onGlobalMediaSearch() {
	...
	$.ajax({
		url:base_url + "/admin/getGlobalMediaData?search="+search,
		success:function(data){
			$('#global_datalist').html(data);
		}
	});
...
function onConfirmMediaDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/onMediaDelete',
...
function onMediaModalView(id) {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/getMediaById',
		data: 'id='+id,
		success: function (response) {
...
function upload_form() {
	...
	if(ext=='jpg' || ext=='JPG' || ext=='jpeg' || ext=='JPEG' || ext=='png' || ext=='PNG' || ext=='gif' || ext=='ico' || ext=='ICO' || ext=='svg' || ext=='SVG'){
		...
		$.ajax({
			url: base_url + '/admin/MediaUpload',
			type: "POST",
			dataType : "json",
			data: data,
			contentType: false,
