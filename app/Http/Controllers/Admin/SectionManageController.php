 ...
 //Section manage page load
    public function getSectionManagePageLoad() {
		...
		$datalist = DB::table('section_manages')
			->join('dp__statuses', 'section_manages.is_publish', '=', 'dp__statuses.id')
			->select('section_manages.*', 'dp__statuses.status')
			->orderBy('section_manages.id','asc')
			->paginate(20);

        return view('admin.section-manage', compact('statuslist', 'datalist'));
    }
	...
	public function getSectionManageTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view('admin.partials.section_manage_table', compact('datalist'))->render();
		}
	}
