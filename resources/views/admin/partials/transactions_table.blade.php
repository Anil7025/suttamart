<div class="table-responsive">
	<table class="table table-borderless table-theme" style="width:100%;">
		...
		<tbody>
			...
			@endphp
			<tr>
				<td class="text-left"><a href="{{ route('admin.order', [$row->id]) }}">{{ $row->order_no }}</a></td>
				<td class="text-left"><a href="{{ route('admin.order', [$row->id]) }}">{{ $row->transaction_no }}</a></td>
				...
				<td class="text-left">{{ $gtext['currency_icon'] }}{{ NumberFormat($total_amount) }}</td>
				@else
				<td class="text-left">{{ NumberFormat($total_amount) }}{{ $gtext['currency_icon'] }}</td>
				@endif
