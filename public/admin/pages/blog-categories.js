...
function onPaginationDataLoad(page) {
	$.ajax({
		url:base_url + "/admin/getBlogCategoriesTableData?page="+page
		+"&search="+$("#search").val(),
...
function onRefreshData() {
	$.ajax({
		url:base_url + "/admin/getBlogCategoriesTableData?search="+$("#search").val(),
		success:function(data){
			$('#tp_datalist').html(data);
			onCheckAll();
		}
...
function onConfirmWhenAddEdit() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/saveBlogCategoriesData',
		data: formData,
        processData: false, 
        contentType: false, 
        beforeSend: function () {
...
function onLoadEditData() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/getBlogCategoriesById',
		data: 'id='+RecordId,
		success: function (response) {
			var data = response;
			...
			if (data.image) {
				$("#view_category_thumbnail").html('<img src="'+base_url+'/uploads/blog-cat/'+data.image+'" class="img-fluid">');
...
function onConfirmDelete() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/deleteBlogCategories',
		data: 'id='+RecordId,
		success: function (response) {
			var msgType = response.msgType;
			var msg = response.msg;
...
function onConfirmBulkAction() {
    ...
    $.ajax({
		type : 'POST',
		url: base_url + '/admin/bulkActionBlogCategories',
...
function onCategorySlug() {
	...
	if(strLength>0){
		$.ajax({
			type : 'POST',
			url: base_url + '/admin/hasBlogCategorySlug',
			data: 'slug='+StrName,
			success: function (response) {
