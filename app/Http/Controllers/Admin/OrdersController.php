...
class OrdersController extends Controller
{
    ...
    public function getOrdersPageLoad() {
		...
		$datalist = DB::table('orders as a')
			...
			->join('order_items as g', 'a.id', '=', 'g.Order_id')
			->select('a.id', 'a.customer_id', 'a.payment_status_id', 'a.order_status_id', 'a.order_no', 'a.created_at', 'a.shipping_fee', DB::raw("SUM(g.total_price) as total_amount"), DB::raw("SUM(g.tax) as tax"), 'b.name', 'd.method_name', 'e.pstatus_name', 'f.ostatus_name')
			->groupBy('a.customer_id', 'a.payment_status_id', 'a.order_status_id', 'a.created_at', 'f.ostatus_name', 'e.pstatus_name', 'd.method_name', 'a.shipping_fee', 'b.name', 'a.order_no', 'a.id')
			->orderBy('a.created_at','desc')
			->paginate(25);
		
        return view('admin.orders', compact('order_status_list', 'datalist'));		
	}
	...
	public function getOrdersTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view('admin.partials.orders_table', compact('datalist'))->render();
		}
	}
	...
	public function bulkActionOrders(Request $request){
		...
		$response = Order::whereIn('id', $idsArray)->update(['order_status_id' => $order_status_id]);
		if($response){
			$res['msgType'] = 'success';
    ...
    public function getOrderData($id) {
        ...
        return view('admin.order', compact('payment_status_list', 'order_status_list', 'mdata', 'datalist'));		
	}	
	...
	public function updateOrderStatus(Request $request){
		...
		$response = Order::where('id', $id)->update($data);
		if($response){
			if($isnotify == 1){
				if($gtext['ismail'] == 1){
					self::orderNotify($id);
    ...
    public function getPaymentOrderStatusData(Request $request) {
		...
		$data = DB::table('orders')
			...
			->select('orders.payment_status_id', 'payment_status.pstatus_name', 'orders.order_status_id', 'order_status.ostatus_name')
			->where('orders.id', $id)
			->first();
	...
	public function deleteOrder(Request $request){
		...
		if($id != ''){
			...
			$response = Order::where('id', $id)->delete();
			if($response){
				$res['msgType'] = 'success';
