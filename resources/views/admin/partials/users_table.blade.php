<div class="table-responsive">
	<table class="table table-borderless table-theme" style="width:100%;">
		<thead>
			<tr>
				<th class="checkboxlist text-center" style="width:4%"><input class="tp-check-all checkAll" type="checkbox"></th>
				<th style="width:12%">{{ __('Name') }}</th>
				<th style="width:12%">{{ __('Email') }}</th>
				<th style="width:12%">{{ __('Phone') }}</th>
				<th style="width:12%">{{ __('Address') }}</th>
				<th style="width:12%">{{ __('Pincode') }}</th>
				<th style="width:12%">{{ __('State') }}</th>
				<th style="width:12%">{{ __('Image') }}</th>
				<th class="text-center" style="width:6%">{{ __('Status') }}</th>
				<th class="text-center" style="width:6%">{{ __('Action') }}</th>
			</tr>
		</thead>
		<tbody>
			@if (count($datalist)>0)
			...
			<tr>
				...
				@if ($row->image != '')
				<td class="text-center"><div class="table_col_image"><img src="{{ asset('uploads/users/' . $row->image) }}" /></div></td>
				@else
