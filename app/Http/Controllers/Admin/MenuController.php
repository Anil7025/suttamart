...
class MenuController extends Controller
{
    ...
    public function getMenuPageLoad(){
		...
		$datalist = DB::table('menus')
			->join('dp__statuses', 'menus.status_id', '=', 'dp__statuses.id')
			->select('menus.id', 'menus.menu_name', 'menus.menu_position','menus.status_id', 'dp__statuses.status')
			->orderBy('menus.id','desc')
			->paginate(10);

        return view('admin.menu', compact('statuslist', 'datalist'));
    }
	...
	public function getMenuTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view('admin.partials.menu_table', compact('datalist'))->render();
		}
	}
