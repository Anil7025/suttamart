...
class SocialMediasController extends Controller
{
    ...
    public function getSocialMediaPageLoad() {
		...
		$datalist = DB::table('social_medias')
			->join('dp__statuses', 'social_medias.is_publish', '=', 'dp__statuses.id')
			->select('social_medias.id', 'social_medias.title', 'social_medias.url', 'social_medias.social_icon', 'social_medias.target', 'social_medias.is_publish', 'dp__statuses.status')
			->orderBy('social_medias.id','desc')
			->paginate(10);

        return view('admin.social-media', compact('statuslist', 'datalist'));
    }
	...
	public function getSocialMediaTableData(Request $request){
		...
		if($request->ajax()){
			...
			return view('admin.partials.social_media_table', compact('datalist'))->render();
		}
	}
